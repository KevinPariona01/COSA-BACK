...
public class CatalogoMapper {
		...
		return new SQL() {
			{
				...
				SELECT("C.TX_VALOR valor");
				SELECT("C.TX_ETIQUETA etiqueta");
				SELECT("C.TX_DESCRIPCION descripcion");
				SELECT("C.TX_ACTIVO activo");
				FROM("CORE_SIEE.T_MAP_CATALOGO C");
    ...
    public String insertar(DTOCatalogoSolicitud solicitud) {
		return new SQL() {
			{
				INSERT_INTO("CORE_SIEE.T_MAP_CATALOGO");
				VALUES("NU_ID_ITEM","t_map_catalogo_seq.nextval");
				VALUES("NU_ID_CATALOGO","#{idCatalogo, jdbcType=INTEGER}");
				VALUES("TX_VALOR","#{valor, jdbcType=VARCHAR}");
				VALUES("TX_ETIQUETA","#{etiqueta, jdbcType=VARCHAR}");
        ...
        return new SQL() {
            {
		        ...
		        WHERE("C.TX_ACTIVO = '1'");
                WHERE("C.TX_REGISTRO_ACTIVO = '1'");
                WHERE("TX_VALOR = #{valor, jdbcType=VARCHAR}");
            }
        }.toString();
    }
